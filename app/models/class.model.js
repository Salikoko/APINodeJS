...
const Class = function(cLass) {
  ...
  this.ID_prim_teacher = cLass.ID_prim_teacher;
};
...
Class.getAll = (ClassName, result) => {
  let query = "SELECT Class.ID_class, Class.Class_Name, Class.Students_Number, Class.AVG_Grades, Teacher.Provided_Subjects, Teacher.Name as 'Primary teacher name' FROM Class";

  if (ClassName) {
    query += ` WHERE Class_Name LIKE '%${ClassName}%'`;
  }

  query+=" JOIN Teacher ON Class.ID_prim_teacher=Teacher.ID_teacher"
...
Class.updateById = (id, cLass, result) => {
  sql.query(
    "UPDATE Class SET Class_Name = ?, Students_Number = ?, AVG_Grades = ?, ID_prim_teacher=? WHERE ID_Class = ?",
    [cLass.Class_Name, cLass.Students_Number, cLass.AVG_Grades,cLass.ID_prim_teacher, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
