...
Student.getAllByClass = (Class_Name,result) => {
  sql.query(`SELECT * FROM Student WHERE ID_student = ${Class_Name}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Student.updateById = (id, student, result) => {
  sql.query(
    "UPDATE Student SET Name = ?, Surname = ?, Class_Name = ? WHERE id = ?",
    [student.Name, student.Surame, student.Class_name, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
